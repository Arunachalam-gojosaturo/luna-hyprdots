...
const GeneralInfo = () => {
  ...
  const updateVersion = async () => {
    try {
      hyprland.dispatch(
        "hl.dsp.exec_cmd('kitty zsh -ic \"clear; luna-hyprdots\"')",
        "",
      );
    } catch (e) {
      ...
      notify({
        ...
        body: errorMessage || "Could not open kitty with luna-hyprdots.",
      });
    }
  };
  ...
  const links = [
    ...
    },
    {
      ...
      url: "https://github.com/Arunachalam-gojosaturo/luna-hyprdots/issues",
      icon: "",
    },
    {
  ...
  return (
    <box class={"info"} orientation={Gtk.Orientation.VERTICAL} spacing={10}>
      <box halign={Gtk.Align.CENTER}>
        <Picture
          file={`${GLib.get_home_dir()}/.config/ags/assets/userpanel/luna-hyprdots_default_pfp.jpg`}
      ...
      <box spacing={10} halign={Gtk.Align.CENTER}>
        <label class={"config-title"} label="Luna-Hyprdots" />
        {/* github stars */}
        <label
          class={"config-stars"}
          $={(self) => {
            execAsync(
              `bash -c "curl -s https://api.github.com/repos/Arunachalam-gojosaturo/luna-hyprdots | jq '.stargazers_count'"`,
            ).then((result) => {
