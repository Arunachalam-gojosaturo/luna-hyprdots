#!/usr/bin/env python3
"""Luna-Hyprdots installation entrypoint."""
...
COUNTER_URL = "https://personal-counter-two.vercel.app/api/increment?workspace=luna-hyprdots&counter=install"
...
def parse_branch(argv: list[str]) -> str:
    ...
    parser.add_argument(
        ...
        "--branch",
        default=None,
        help="Luna-Hyprdots git branch to clone (default: master)",
    )
    parser.add_argument(
        "legacy_branch",
        nargs="?",
    ...
    if os.environ.get("LUNA_HYPRDOTS_BRANCH"):
        return str(os.environ["LUNA_HYPRDOTS_BRANCH"])
    return "master"


def main() -> None:
    conf_dir = Path.home() / "luna-hyprdots"
    ...
    print("Cloning Luna-Hyprdots repository (latest commit only)...")
    run_cmd(
        [
            "git",
            "clone",
            ...
            "--branch",
            branch,
            "https://github.com/Arunachalam-gojosaturo/luna-hyprdots.git",
            str(conf_dir),
        ]
    )
    ...
    plan = presentation.collect_section_choices(
        ...
        [
            ...
            presentation.PlannedStep(
                "config", "Applying Luna-Hyprdots Configuration", default_choice="y"
            ),
            presentation.PlannedStep(
                "keyboard",
    ...
    presentation.print_section_header("SYSTEM TWEAKS")
    presentation.execute_planned_step(
        "*",
